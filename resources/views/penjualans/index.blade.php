...
<body>
    ...
    <div class="container mx-auto mt-10 p-6 bg-white rounded-lg shadow-lg">
        <h1 class="text-3xl font-bold text-blue-900 mb-6 text-center">Data Penjualan</h1>
    
        <form action="{{ route('penjualans.index') }}" method="GET" class="mb-4">
        ...
        <div class="overflow-x-auto">
            <table class="w-full border border-blue-400 bg-white shadow-lg rounded-lg">
                <thead>
                    <tr class="text-center bg-blue-600 text-white">
                        ...
                        <th class="px-4 py-3 border">Metode Pembayaran</th>
                    ...
                    @foreach($penjualans as $index => $penjualan)
                        ...
                        @foreach($penjualan->detailPenjualan as $key => $detail)
                            <tr class="text-center hover:bg-blue-50 transition">
                                @if($key == 0)
                                    <td class="px-4 py-3 border align-middle" rowspan="{{ $rowspan }}">{{ $index + 1 }}</td>
                                    <td class="px-4 py-3 border text-left w-40 whitespace-nowrap" rowspan="{{ $rowspan }}">{{ $penjualan->TanggalPenjualan }}</td>
                                    <td class="px-4 py-3 border text-left w-40 whitespace-nowrap" rowspan="{{ $rowspan }}">{{ $penjualan->pelanggan->NamaPelanggan ?? '-' }}</td>
                                @endif
                
                                <td class="px-4 py-3 border text-left w-40 whitespace-nowrap">{{ $detail->produk->NamaProduk ?? '-' }}</td>
                                <td class="px-4 py-3 border text-center">{{ $detail->JumlahProduk }}</td>
                                <td class="px-4 py-3 border text-right">Rp {{ number_format($detail->SubTotal, 2, ',', '.') }}</td>
                
                                @if($key == 0)
                                    <td class="px-4 py-3 border align-middle" rowspan="{{ $rowspan }}">Rp {{ number_format($penjualan->TotalHarga, 2, ',', '.') }}</td>
                                    <td class="px-4 py-3 border align-middle" rowspan="{{ $rowspan }}">{{ $penjualan->pembayaran->MetodePembayaran ?? 'Cash' }}</td>
                                    <td class="px-4 py-3 border align-middle flex flex-col gap-1 items-center" rowspan="{{ $rowspan }}">
